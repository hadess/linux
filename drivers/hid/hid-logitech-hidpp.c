...
#include <linux/kern_levels.h> //FIXME
						     ...
						     int *level)
{
	...
	if (ret == HIDPP_ERROR_RESOURCE_ERROR)
		return -EIO;
	if (ret > 0) {
		if (ret != HIDPP20_ERROR_LOGITECH_INTERNAL)
			hid_err(hidpp->hid_dev, "%s: received protocol error 0x%02x\n",
				__func__, ret);
		else
			hid_err(hidpp->hid_dev, "%s: received protocol error 0x%02x\n", //FIXME
...
static int hidpp20_map_adc_measurement_1f20(u8 data[3], int *voltage)
{
	...
	flags = (int) data[2];
	printk(KERN_ERR "%s: got 1f20 flag 0x%02x\n", __func__, flags); //FIXME
	...
	*voltage = get_unaligned_be16(data);
	printk(KERN_ERR "%s: got 1f20 voltage %dmV\n", __func__, *voltage); //FIXME
